package com.runninghi.runninghibackv2.replyreport.application.controller;
...
import com.runninghi.runninghibackv2.replyreport.application.service.ReplyReportService;
...
public class ReplyReportController {
    ...
    @GetMapping("/reply")
    public ResponseEntity<ApiResult> getReplyReportsByReplyId(@RequestParam(defaultValue = "0") @PositiveOrZero int page,
    ...
    @PutMapping("/reply")
    public ResponseEntity<ApiResult> handleReplyReports(@RequestParam boolean isAccepted,
                                                        @RequestParam Long replyNo) {
    ...
    public ResponseEntity<ApiResult> deleteReplyReport(@PathVariable Long replyReportNo) {

        replyReportService.deleteReplyReport(replyReportNo);

        return ResponseEntity.ok(ApiResult.success("댓글신고 삭제 성공", null));
    }
}
